...
public class Celula extends Observable implements Callable<Individuo>{
	...
	private PSOVizinhanca psoVizinhanca;
	...
	public Celula(VizinhancaStrategy vizinhanca, SincronizadorGeracao m, int x, int y, PSOVizinhanca psoVizinhanca, String numeroCelula, int qtdGeracoes,Populacao populacao, RandomGenerator random,  
	...
	public Individuo executar() {
		...
		for (int geracao = 0; !criteriosParada.done(populacao, geracao); geracao++) {
			...
			individuosVizinhanca.add(this.getIndividuo());
			...
			for(int i=0;i<populacao.getTamanhoPopulacao();i++){
				...
				avaliador.avaliarPopulacao(populacao);
				if (populacao.getIndividuo(i).getAptidao() < this.getIndividuo().getAptidao()) {
					
					populacao.setIndividuo(populacao.getIndividuo(i));
